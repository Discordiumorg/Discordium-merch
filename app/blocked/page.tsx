    ...
    <div className="min-h-screen bg-brand-dark pb-28">
      <div className="sticky top-0 z-30 bg-brand-dark/90 backdrop-blur-xl border-b border-white/10 px-5 pt-12 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-white font-black text-xl">Blockierte Nutzer</h1>
          </div>
          {blocked.length > 0 && (
      ...
      <div className="px-5 py-4 space-y-4">
        <div className="card-glass rounded-2xl p-3.5 border border-yellow-500/20 flex items-start gap-3">
          ...
          <p className="text-yellow-300/80 text-xs leading-relaxed">Blockierte Nutzer können dein Profil nicht sehen und dir keine Nachrichten senden. Das Entblockieren stellt den vorherigen Status wieder her.</p>
        </div>
          ...
          {blocked.map((user, i) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                >
                  <RotateCcw size={12} /> Entblockieren
                </motion.button>
              </div>
            </motion.div>
          ))}
        ...
        {blocked.length === 0 && (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-16">
            ...
            <p className="text-white/50 text-base font-semibold">Keine blockierten Nutzer</p>
            <p className="text-white/30 text-sm mt-1">Du hast niemanden blockiert.</p>
          </motion.div>
        )}
      </div>
        ...
        {showConfirm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-end justify-center p-4">
            <motion.div initial={{ y: 100 }} animate={{ y: 0 }} exit={{ y: 100 }} transition={{ type: 'spring', stiffness: 340, damping: 28 }} className="bg-brand-card rounded-3xl p-6 w-full max-w-md">
              <p className="text-white font-black text-center text-lg mb-2">Nutzer entblockieren?</p>
              <p className="text-white/50 text-sm text-center mb-5">{blocked.find(u => u.id === showConfirm)?.name} kann dich dann wieder sehen und kontaktieren.</p>
              <button onClick={() => unblock(showConfirm)} className="w-full gradient-brand text-white font-bold py-3 rounded-2xl glow-button mb-2">Entblockieren</button>
              <button onClick={() => setShowConfirm(null)} className="w-full bg-white/10 text-white/70 py-3 rounded-2xl">Abbrechen</button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
